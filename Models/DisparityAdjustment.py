...
def disparity_adjustment(orig_image, disparity_map_batch):
    boolUsed = {}
    tenMasks = []
    adjusted_disp = []
    for i in range(orig_image.shape[0]):
        disparity_map = disparity_map_batch[0,:,:,:].unsqueeze(0)
        objPredictions = netMaskrcnn([ orig_image[ i, [ 2, 0, 1 ], :, : ] ])[0]
        ...
        adjusted_disp.append(torch.nn.functional.interpolate(input=tenAdjusted, size=(disparity_map.shape[2], disparity_map.shape[3]), mode='bilinear', align_corners=False))
         
    return torch.cat(adjusted_disp)