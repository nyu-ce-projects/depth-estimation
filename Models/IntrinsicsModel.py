...
class IntrinsicsModel(nn.Module):
    ...
    def forward(self, bottleneck, img_width, img_height):
        ...
        intrinsics_mat = torch.eye(4).unsqueeze(0)
        intrinsics_mat = intrinsics_mat.repeat(batch_size, 1, 1)

        focal_lengths = (self.softplus(self.focal_length_conv(bottleneck)).squeeze() * torch.Tensor([img_width, img_height]))
        offsets = ((self.softplus(self.offsets_conv(bottleneck)).squeeze() + 0.5) * torch.Tensor([img_width, img_height])).unsqueeze(-1)
